...
class Router {
  ...
  #addRoute(path, method, handler, middleware = []) {
    ...
    currentNode.middleware = middleware; // Store route-specific middleware
  }
  ...
  async #executeMiddlewareAndHandler(middleware, handler, req, res) {
    const context = {}; // Shared context object
    const execute = async (index) => {
      if (index < middleware.length) {
        await middleware[index](req, res, context, () => execute(index + 1));
      } else {
