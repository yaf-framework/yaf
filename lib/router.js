...
class Yaf {
  ...
  constructor() {
    ...
    Object.keys(HTTP_METHODS).forEach((methodKey) => {
      ...
      Yaf.prototype[httpMethod.toLowerCase()] = function (path, ...handlers) {
  ...
  async handleRequest(req, res) {
    // Read the request data and store it in req.rawData
    req.rawData = await new Promise((resolve, reject) => {
      let chunks = [];
