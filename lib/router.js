...
class Response {
  ...
  json(data) {
    this.res.writeHead(this.statusCode, { ...this.headers, 'Content-Type': 'application/json' });
  ...
  send(data) {
    if (typeof data === 'object') {
      ...
      this.res.writeHead(this.statusCode, { ...this.headers, 'Content-Type': 'text/plain' });
      this.res.end(data);
    }
  }

  redirect(url) {
    this.res.writeHead(302, { ...this.headers, 'Location': url });
    this.res.end();
  }
}
...
class Router {
  ...
  #addRoute(path, method, handler, middleware = []) {
    ...
    // Step 1: Combine global, group, and route-specific middleware
    const allMiddleware = [
      ...this.globalMiddleware,
      ...this.middlewareStack.flatMap(group => group.middleware),
      ...middleware,
    ].flat(); // Flatten the array
  
    // Step 2: Apply path prefix from the middleware stack
  ...
  async #executeMiddlewareAndHandler(middleware, handler, req, res) {
    ...
    const execute = async (index) => {
      if (index < middleware.length) {
        ...
        // Check if middleware expects context parameter
        if (currentMiddleware.length <= 3) {
          await currentMiddleware(req, res, () => execute(index + 1));
        } else {
          await currentMiddleware(req, res, context, () => execute(index + 1));
        }
      } else {
  ...
  // Define the group method to:

  // Accept a path prefix (optional) and middleware array.
